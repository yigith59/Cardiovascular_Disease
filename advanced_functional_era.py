...
kategorik, sayisal, sayisal_kategorik, kategorik_kardinal,tarih = degisken_siniflama(df)
    ...
    for col in cols:
        if plot:
            ...
            if df[col].dtype == bool:
                fig, axes = plt.subplots(1, 2, figsize=(12, 5))  # 1 satırda 2 ayrı grafik

                sns.countplot(x=df[col].astype(int), ax=axes[0],palette="muted",hue=df[col],legend=False)  # İlk grafiği sol tarafa koy
                axes[0].set_title(f"{col} - Countplot")

                df[col].value_counts().plot.pie(autopct="%1.1f%%", colormap="Set3",
                                                ax=axes[1])  # İkinci grafiği sağ tarafa koy
                ...
                sns.countplot(x=df[col], ax=axes[0],palette="muted",hue=df[col],legend=False)  # İlk grafiği sol tarafa koy
                axes[0].set_title(f"{col} - Countplot")
...
def sayisal_ozet(df,cols,plot=False):
    # tekli veya çoklu kullanım
    # Aykırı Değer Sayısı ve Yüzdesi: Hesaplanan eşiklere göre gerçek aykırı değerlerin sayısını ve yüzdesini de yazdırmak faydalı olabilir.
    ...
    for col in cols:
        ...
        if plot:
            ...
            sns.histplot(df[col], kde=True, ax=ax[1], color="purple", bins=30)  # KDE eklenmiş histogram
...
def threshold_hesaplama(df,col, a=0.25, b=0.75):
    temp_df = df.copy()
    outlier_threshold_df = pd.DataFrame()
    ...
    if type(col) == list:
        for i in col:
            q1 = temp_df[i].quantile(a)
            q3 = temp_df[i].quantile(b)
            iqr = q3 - q1
            ...
            deger_df = pd.DataFrame({"min_deger": min_deger,
                                          "max_deger": max_deger},index=[i])
            outlier_threshold_df = pd.concat([outlier_threshold_df,deger_df])

        print(outlier_threshold_df,end="\n\n")
        return outlier_threshold_df,df[col].describe().T
        ...
        print(outlier_threshold_df,end="\n\n")
        return outlier_threshold_df,min_deger,max_deger

def outlier_kontrol(df,col,index=False):
    ...
    temp_df = df.copy()
    outlier_threshold_df, min_deger, max_deger = threshold_hesaplama(df,col)
...
def outlier_cozum(df,col,index_del=False, secim=False, baskilama=False):
...
def eksik_veriye_bakis(df, index=False):
...
def eksik_veri_cozum(df, tek_kolon=False, istenen_tek_col=False, cok_kolon=False,
    ...
    eksik_kolonlar, na_df = eksik_veriye_bakis(df)
...
def standart_scaler(df, col):
    ss = StandardScaler()
    df[col + "_std_sclr"] = ss.fit_transform(df[[col]])
    return ss

def robust_scaler(df, col):
    rs = RobustScaler()
    df[col + "_rbst_sclr"] = rs.fit_transform(df[[col]])
    return rs

def min_max_scaler(df, col):
    mm = MinMaxScaler()
    df[col + "_min_max_sclr"] = mm.fit_transform(df[[col]])
    return mm
